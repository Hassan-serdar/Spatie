                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('users.update', $user->id) }}" method="POST">
                        ...
                        <div>
                            ...
                            <div class="my-3">
                                <input name="name" value="{{ old('name', $user->name) }}" type="text" placeholder="Enter Name"
                                    class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                            </div>

                            <label for="email" class="text-lg font-medium">Email</label>
                            <div class="my-3">
                                <input name="email" value="{{ old('email', $user->email) }}" type="text" placeholder="Enter Email"
                                    class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                            </div>
                            ...
                            <div class="grid grid-cols-4 mb-3">
                                @if ($roles->isNotEmpty())
                                    @foreach ($roles as $role)
                                        <div class="mt-3">
                                            <input {{ ($hasRoles->contains($role->name)) ? 'checked' : '' }}
